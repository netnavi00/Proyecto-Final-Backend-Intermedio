...
{/* Información adicional (departamento y fecha de ingreso)*/}
      <div className="mt-5 space-y-2.5">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-md text-xs font-black uppercase tracking-wider">
...
{/* Fecha de ingreso*/}
        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="text-xs font-medium">
            Ingreso: {employee.hire_date ? new Date(employee.hire_date).toLocaleDateString() : '---'}
          </span>
        </div>
      </div>
