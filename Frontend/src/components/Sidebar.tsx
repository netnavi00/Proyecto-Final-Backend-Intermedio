...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className={cn(
      "bg-slate-900 text-slate-300 flex flex-col h-screen sticky top-0 transition-all duration-300 ease-in-out relative shadow-2xl z-50",
      ...
      <button 
        ...
        className="absolute -right-5 bottom-8 w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white border-4 border-slate-900 hover:bg-indigo-700 transition-colors z-50 shadow-xl shadow-slate-950/30"
      >
        <ChevronLeft className={cn(
          "w-6 h-6 transition-transform duration-300", // Icono más grande (w-6 h-6)
      ...
      <div className="p-6 border-b border-slate-800">
        <div className={cn("flex items-center gap-3 px-2", isCollapsed && "justify-center")}>
          ...
          {!isCollapsed && (
            <h1 className="text-xl font-black text-white tracking-tighter animate-in fade-in duration-300">
